  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-white truncate group-hover:text-indigo-300 transition-colors">{coordinator.name}</p>
...
function PrizeCard({
  ...
  const styles = {
    ...
    bronze: "from-orange-700/10 to-transparent border-orange-700/20 text-orange-600",
  };

  return (
    <div className={`relative rounded-xl border bg-gradient-to-b p-4 ${styles[color]}`}>
      <div className="flex items-center gap-3">
        <div className={`p-2 rounded-lg bg-white/5 ${styles[color].split(" ").pop()}`}>
            {icon}
        </div>
        <div>
      ...
      <div className="min-h-screen text-white selection:bg-indigo-500/30">
        ...
        */}
        <main className="relative z-10 lg:flex max-w-6xl mx-auto">
            ...
            <section className="w-full lg:w-[45%] xl:w-[45%] lg:h-screen lg:sticky lg:top-0 p-4 sm:p-6 flex flex-col gap-5 overflow-y-auto no-scrollbar">
                
                {/* Back Button */}
                <Link href="/events" className="inline-flex items-center gap-2 text-sm text-white/50 hover:text-white transition-colors w-fit">
                ...
                <motion.div 
                    ...
                    className="relative aspect-[4/5] w-full max-w-lg mx-auto lg:max-w-none rounded-2xl overflow-hidden border border-white/10 shadow-2xl shadow-indigo-500/10 group"
                >
                    <Image
                        src={event.image}
                 ...
                 <div className="hidden lg:grid gap-4 mt-auto">
                    {event.coordinators && event.coordinators.length > 0 && (
                        <div className="space-y-2">
                             <p className="text-xs text-white/40 font-medium uppercase tracking-wider">Event Coordinators</p>
                             <div className="grid grid-cols-1 xl:grid-cols-2 gap-2">
                                {event.coordinators.map((c, i) => (
                                    <CoordinatorCard key={i} coordinator={c} eventTitle={event.title} />
                                ))}
                             </div>
                        </div>
                    )}
            ...
            <section className="w-full lg:w-[55%] xl:w-[55%] p-4 sm:p-6 lg:pt-16 pb-24 space-y-8">
               ...
               {(event.firstPrize || event.secondPrize) && (
                   ...
                   >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            {event.firstPrize && <PrizeCard prize={event.firstPrize} title="1st Place" color="gold" icon={<LuTrophy />} />}
                            {event.secondPrize && <PrizeCard prize={event.secondPrize} title="2nd Place" color="silver" icon={<LuAward />} />}
                        </div>
                   </motion.div>
               )}
                                ...
                                {Array.isArray(event.rules) ? (
                                    (event.rules as string[]).map((rule, i: number) => (
                                        <li key={i} className="flex gap-3 text-white/80 text-sm leading-relaxed">
                                            <span className="text-fuchsia-500 font-bold mt-1">•</span>
                                            <span>{rule}</span>
                                        </li>
                                    ))
                                ) : (
               ...
               <div className="lg:hidden space-y-4 pt-8 border-t border-white/10">
                    {event.coordinators && event.coordinators.length > 0 && (
                        <>
                             <p className="text-sm text-white/40 font-medium uppercase tracking-wider">Event Coordinators</p>
